...
bool DataProcess::processBasicStatusMessage(char *in,RadarBasicStatus *basic_status)
{
    ...
    basic_status->Timestamp_Nanoseconds = cvt.byteToUint(in + 8);
    basic_status->Timestamp_Seconds = cvt.byteToUint(in + 12);
    basic_status->Timestamp_SyncStatus = (unsigned char)(in[16]);
    basic_status->SWVersion_Major = (unsigned char)(in[17]);
    basic_status->SWVersion_Minor = (unsigned char)(in[18]);
    basic_status->SWVersion_Patch = (unsigned char)(in[19]);

    basic_status->Longitudinal = cvt.byteToFloat(in + 20);
    basic_status->Lateral = cvt.byteToFloat(in + 24);
    basic_status->Vertical = cvt.byteToFloat(in + 28);
    basic_status->Yaw = cvt.byteToFloat(in + 32);
    basic_status->Pitch = cvt.byteToFloat(in + 36);
    basic_status->PlugOrientation = (unsigned char)(in[40]);
    basic_status->Length = cvt.byteToFloat(in + 41);
    basic_status->Width = cvt.byteToFloat(in + 45);
    basic_status->Height = cvt.byteToFloat(in + 49);
    basic_status->Wheelbase = cvt.byteToFloat(in + 53);
    basic_status->MaximumDistance = cvt.byteToUshort(in + 57);
    basic_status->FrequencySlot = (unsigned char)(in[59]);
    basic_status->CycleTime = (unsigned char)(in[60]);
    basic_status->TimeSlot = (unsigned char)(in[61]);
    basic_status->HCC = (unsigned char)(in[62]);
    basic_status->Powersave_Standstill = (unsigned char)(in[63]);
    basic_status->SensorIPAddress_0 = cvt.byteToUint(in + 64);
    basic_status->SensorIPAddress_1 = cvt.byteToUint(in + 68);
    basic_status->Configuration_counter = (unsigned char)(in[72]);
    basic_status->Status_LongitudinalVelocity = (unsigned char)(in[73]);
    basic_status->Status_LongitudinalAcceleration = (unsigned char)(in[74]);
    basic_status->Status_LateralAcceleration = (unsigned char)(in[75]);
    basic_status->Status_YawRate = (unsigned char)(in[76]);
    basic_status->Status_SteeringAngle = (unsigned char)(in[77]);
    basic_status->Status_DrivingDirection = (unsigned char)(in[78]);
    basic_status->Status_CharacteristicSpeed = (unsigned char)(in[79]);
    basic_status->Status_Radar = (unsigned char)(in[80]);

    basic_status->Status_VoltageStatus = (unsigned char)(in[81]);
    basic_status->Status_TemperatureStatus = (unsigned char)(in[82]);
    basic_status->Status_BlockageStatus = (unsigned char)(in[83]);
